...
{
    readonly List<WindowsMediaPlayer> wmps = [];

    private readonly WindowsMediaPlayer SingleWMP = new();
    ...
    private void ChooseFolder(string initialPath)
    {
        ...
        else
        {
            try
            {
                using FolderBrowserDialog fbd = new() { SelectedPath = initialPath };
                if (fbd.ShowDialog() == DialogResult.OK)
                {
                    Settings.Default.SoundsDir = fbd.SelectedPath;
                    Settings.Default.Save();
    ...
    private void SearchText_TextChanged(object sender, EventArgs e)
    {
        ...
        DrawButtons(buttons: [.. result], QueriedPanel);
    }
